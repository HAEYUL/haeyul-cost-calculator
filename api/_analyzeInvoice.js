  ...
  items: z
    .array(
      z.object({
        ...
        unit: z
          .enum(['g', 'kg', 'ea', 'box', 'other'])
          .nullable()
          .describe(
            '단가가 무엇을 기준으로 매겨졌는지. g=그램당, kg=킬로그램당, ea=개(마리/봉지 등 낱개)당, box=박스(상자) 단위당, ' +
              'other=그 외. 알 수 없으면 null',
          ),
        ...
        vat: z
          ...
          .nullable()
          .describe(
            '이 품목 행에 "부가세", "세액", "부가가치세"처럼 따로 적힌 부가세 금액. 공급가액(금액)과 합쳐서 이미 ' +
...
- 각 품목의: 물품명, 수량, 단가, 단위, 금액, 부가세(따로 적혀있으면)
...
- 단위(unit)는 단가가 무엇 기준인지를 나타냅니다. 예: "13,500원/kg"이면 kg, "돼지고기 1kg 13,500원"처럼 수량 단위가 kg면 kg, "계란 30개 12,000원"처럼 개수 기준이면 ea, "10박스" 처럼 박스 단위면 box, 그램 기준이면 g. 명확하지 않으면 null로 두세요.
- 부가세(vat)는 그 품목 행에 부가세 금액이 별도 칸으로 적혀 있을 때만 채우고, 그런 칸 자체가 없으면 null로 두세요.
